...
function CompareModal({ players, onClose }) {
  ...
  const rows = [
    { label: "Role",          va: a.role,          vb: b.role },
    { label: "Base Price",    va: formatCurrency(a.base_price), vb: formatCurrency(b.base_price) },
    { label: "Category",      va: a.category_name, vb: b.category_name },
    { label: "Country",       va: a.country_name,  vb: b.country_name },
    { label: "Age",           va: a.age ? `${a.age} yrs` : "—", vb: b.age ? `${b.age} yrs` : "—" },
    { label: "Batting Style", va: a.batting_style || "—", vb: b.batting_style || "—" },
    { label: "Bowling Style", va: a.bowling_style || "—", vb: b.bowling_style || "—" },
  ];
  ...
  return (
    <Modal open={true} onClose={onClose} title="Player Comparison" width={600}>
      ...
      <div className="space-y-1">
        {rows.map((row) => (
          <div key={row.label} className="grid grid-cols-3 items-center gap-2 py-2.5 border-b border-slate-50 last:border-0">
            <p className="text-ui text-slate-900">{row.va || "—"}</p>
            <p className="text-sub text-slate-900 text-center">{row.label}</p>
            <p className="text-ui text-slate-900 text-right">{row.vb || "—"}</p>
          </div>
        ))}
      </div>
    </Modal>
    ...
    <Modal open={true} onClose={onClose} title="Wishlist Settings" width={400}>
      <div className="flex items-center gap-3 mb-5 p-3 rounded-md bg-slate-50">
        ...
        <div>
          ...
          <p className="text-sub text-slate-900">{player.role} · Base: {formatCurrency(player.base_price)}</p>
        </div>
      </div>
      
      <div className="mb-5">
        <p className="text-ui font-semibold text-slate-900 mb-2">Target Priority</p>
        <div className="flex gap-2">
          {["primary", "secondary", "avoid"].map((p) => (
            <button
              key={p}
      ...
      <div className="mb-2">
        <p className="text-ui font-semibold text-slate-900 mb-2">Max Bid Limit (Optional)</p>
        <Input
          value={val}
          onChange={(e) => setVal(e.target.value.replace(/[^0-9]/g, ""))}
          placeholder="e.g. 75000000"
          icon={CoinVertical}
          disabled={priority === "avoid"}
        />
        {val && priority !== "avoid" && (
          <p className="text-ui-xs text-slate-400 mt-2">= {formatCurrency(Number(val))}</p>
        )}
      </div>
      ...
      <div className="mt-6 flex justify-end gap-3">
        ...
        <Button 
          ...
          loading={loading}
          loadingText="Saving..."
          onClick={() => onSave({ max_bid: priority === "avoid" ? null : Number(val) || null, priority })}
        >
          Save Settings
        </Button>
      </div>
...
export default function FranchisePoolPage() {
  ...
  const [compareList, setCompareList] = useState([]); // max 2
  ...
  const fetchAll = () => {
    apiFetch("/franchise/pool").then(setPool).catch((err) => toast("Failed to load auction pool: " + err.message, "error"));
    apiFetch("/franchise/wishlist").then(setWishlist).catch(() => {});
  };

  useEffect(() => { fetchAll(); }, []);

  const toggleWishlist = async (player_id) => {
    setToggling(player_id);
    try {
      await apiFetch("/franchise/wishlist/toggle", { method: "POST", body: JSON.stringify({ player_id }) });
      fetchAll();
    } catch (err) { 
  ...
  const saveSettings = async (player_id, settings) => {
    setSaving(true);
    try {
      await apiFetch("/franchise/wishlist/max-bid", { method: "PATCH", body: JSON.stringify({ player_id, max_bid: settings.max_bid }) });
      await apiFetch("/franchise/wishlist/priority", { method: "PATCH", body: JSON.stringify({ player_id, priority: settings.priority }) });
      setSettingsPlayer(null);
      fetchAll();
      toast("Target settings updated successfully.", "success");
    } catch (err) { 
  ...
  const toggleCompare = (item) => {
    setCompareList((prev) => {
      if (prev.find((p) => p.player_id === item.player_id)) return prev.filter((p) => p.player_id !== item.player_id);
      if (prev.length >= 2) { toast("Only 2 players can be compared at once.", "error"); return prev; }
      return [...prev, item];
    });
  };

  // Merge pool data with wishlist max_bid & priority
  const poolWithMeta = useMemo(() => {
    const wMap = {};
    wishlist.forEach((w) => { wMap[w.player_id] = w; });
    return pool.map((p) => ({
      ...p,
      is_wishlisted: !!wMap[p.player_id],
      max_bid: wMap[p.player_id]?.max_bid || null,
      priority: wMap[p.player_id]?.priority || p.wishlist_priority || "primary",
    }));
  }, [pool, wishlist]);

  // Budget planner totals (Exclude 'avoid' targets)
  const budgetPlannerTotal = useMemo(
    () => wishlist.filter(w => w.priority !== 'avoid').reduce((sum, w) => sum + Number(w.max_bid || 0), 0),
    [wishlist]
  );

  const filtered = poolWithMeta.filter((item) =>
    [item.name, item.role, item.category_name].filter(Boolean).join(" ").toLowerCase().includes(search.toLowerCase())
  );
  ...
  return (
    <DashboardLayout allowedRoles={["Franchise"]}>
      ...
      {showCompare && compareList.length === 2 && (
        <CompareModal players={compareList} onClose={() => setShowCompare(false)} />
      )}
      {settingsPlayer && (
        <WishlistSettingsModal
          player={settingsPlayer}
          currentMax={settingsPlayer.max_bid}
          currentPriority={settingsPlayer.priority}
          onClose={() => setSettingsPlayer(null)}
          onSave={(settings) => saveSettings(settingsPlayer.player_id, settings)}
          loading={saving}
        />
      )}
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-h1 text-slate-900">Auction Pool</h1>
          <p className="text-sub text-slate-900">View all queued players, set targets and max bids, compare candidates.</p>
        </div>
        {compareList.length === 2 && (
          <Button variant="primary" onClick={() => setShowCompare(true)}>
            <ArrowsLeftRight size={16} /> Compare Selected
          </Button>
        )}
      </div>
      ...
      {wishlist.length > 0 && (
        <div className="mb-6 grid gap-4 sm:grid-cols-3">
          <SectionCard title="Targets Set" sub="Players in your watchlist.">
            <p className="text-h2 text-slate-950 mt-1">{wishlist.length}</p>
          </SectionCard>
          <SectionCard title="Committed Budget" sub="Sum of all max bids set.">
            <p className="text-h2 text-slate-950 mt-1">{budgetPlannerTotal > 0 ? formatCurrency(budgetPlannerTotal) : "—"}</p>
          </SectionCard>
          <SectionCard title="Players with Max Bid" sub="Budget limits configured.">
            <p className="text-h2 text-slate-950 mt-1">{wishlist.filter((w) => w.max_bid).length} / {wishlist.length}</p>
          </SectionCard>
        </div>
      )}
      ...
      <AnimatePresence>
        {compareList.length > 0 && (
          <motion.div
            ...
            className="mb-4 flex items-center justify-between p-3 rounded-md bg-slate-900 text-white"
          >
            <p className="text-ui font-medium">
              {compareList.length === 1
                ? `${compareList[0].name} selected — pick 1 more to compare`
                : `${compareList[0].name} vs ${compareList[1].name}`}
            </p>
            <div className="flex gap-2">
              {compareList.length === 2 && (
                <Button size="sm" variant="outline" className="border-white/20 text-white hover:bg-white/10" onClick={() => setShowCompare(true)}>
                  Compare Now
                </Button>
              )}
                  ...
                  {paginated.map((item) => {
                    ...
                    return (
                      <TableRow key={item.player_id} className={inCompare ? "bg-slate-50" : ""}>
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <div className="flex items-center gap-2">
                                {item.country_code && <img src={`https://flagcdn.com/w20/${item.country_code.toLowerCase()}.png`} className="w-4 h-3 object-contain" />}
                        ...
                        <TableCell>
                          <span className={cn("badge gap-1", statusClass(item.status))}>
                          ...
                          {item.is_wishlisted ? (
                            <div className="flex flex-col gap-1 items-start">
                              {item.priority === "avoid" ? (
                                ...
                                <button
                                  onClick={() => setSettingsPlayer(item)}
                                  className={cn("text-ui-xs font-bold px-2 py-1 rounded-md border transition-colors",
                                    item.max_bid
                      ...
                      <div className="p-5">
                        ...
                        <div className="grid grid-cols-2 gap-2 py-3 border-y border-slate-50 mb-4">
                          <div>
                            ...
                            <span className="text-ui-semibold text-slate-900">{formatCurrency(item.base_price)}</span>
                          </div>
                          <div className="text-right">
                            <span className="block text-[9px] text-slate-400 font-bold uppercase">Lot</span>
                            <span className="text-ui text-slate-400">#{item.lot_number}</span>
                          </div>
                        </div>
                        {item.is_wishlisted && (
                          <div className="flex items-center gap-2">
