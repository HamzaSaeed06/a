...
export default function FranchiseLiveAuction() {
  ...
  useEffect(() => {
    ...
    socket.on('player_sold', (data) => {
      addToast(`${data.player.name} sold to ${data.team.team_name} for ${formatCurrency(data.amount)}`, "success");
      fetchInitialData();
    });
  ...
  return (
    <DashboardLayout title="Live Auction War Room">
      ...
      <div className="flex flex-col gap-6">
         ...
         {/* TOP SECTION: LIVE FLOOR */}
         {!currentPlayer ? (
            /* Professional Placeholder when Session is Idle */
            <div className="surface bg-white py-16 px-10 flex flex-col items-center justify-center text-center border border-slate-100 shadow-sm rounded-lg relative overflow-hidden">
               ...
               <div className="h-14 w-14 rounded-full bg-slate-50 flex items-center justify-center mb-5">
                  <Broadcast size={28} weight="duotone" className="text-slate-400" />
               </div>
               <h2 className="text-ui-semibold text-slate-900 capitalize mb-1.5">Auction floor is currently idle</h2>
               <p className="text-slate-400 max-w-sm mx-auto text-[11px] leading-relaxed">
                  The session is waiting for the administrator to introduce the next player. Live bid updates will appear here automatically.
               </p>
            </div>
         ) : (
              <div className="grid gap-8 lg:grid-cols-12 items-stretch min-h-[580px]">
                 ...
                 <div className="lg:col-span-4">
                    <SectionCard padded={false} className="surface h-full flex flex-col bg-white overflow-hidden shadow-sm group">
                       <div className="flex-1 p-8 pb-4 flex flex-col items-center">
                          ...
                          <div className="mt-8 text-center">
                             ...
                             <p className="text-ui font-medium text-slate-500 capitalize">{currentPlayer.role_name || currentPlayer.role}</p>
                          </div>
                       </div>
                 ...
                 <div className="lg:col-span-8">
                    <SectionCard className="surface h-full bg-white flex flex-col items-center justify-center p-12 relative overflow-hidden">
                       ...
                       <div className="text-center relative z-10 w-full max-w-md">
                          ...
                          <p className="text-sub text-slate-400 tracking-[0.2em] mb-12 uppercase">Highest Floor Bid</p>
                          ...
                          <div className="flex items-center justify-center gap-3 mb-12">
                             <div className={cn(
                                ...
                                {highestBidder?.team_id === myTeam?.team_id ? <CheckCircle size={22} weight="fill" /> : <MonitorPlay size={22} weight="fill" />}
                             </div>
                             <span className="text-xl font-bold text-slate-900 tracking-tight">
                                {highestBidder ? (
                                   highestBidder.team_id === myTeam?.team_id ? "Your Bid is Leading" : `Leading: ${highestBidder.team_name}`
                                ) : "Awaiting First Bid"}
                             </span>
                          </div>
                          ...
                          <div className="flex items-center gap-4 mb-10 px-4">
                             ...
                             <div className="flex-1 h-14 rounded-xl border border-slate-900 bg-white flex items-center justify-center shadow-[inset_0_2px_4px_rgba(0,0,0,0.02)]">
                                <span className="text-xl font-bold text-slate-900 tabular-nums">{formatCurrency(bidAmount)}</span>
                             </div>
                             <button 
                          ...
                          <div className="w-full relative h-1.5 bg-slate-100 rounded-full mb-6 overflow-hidden">
                             <motion.div 
                                className={cn(
                                   ...
                                   isUrgent ? "bg-red-500" : isWarning ? "bg-amber-500" : "bg-slate-950"
                                )}
                                initial={{ width: "100%" }}
                          ...
                          <div className="flex items-center justify-center gap-2 text-slate-500">
                             <Clock size={20} weight="bold" className={cn(isUrgent && "text-red-500 animate-pulse")} />
                             <span className={cn("text-xs font-bold uppercase tracking-[0.2em]", isUrgent && "text-red-500 font-black")}>
                          ...
                          <div className="mt-12 w-full px-4">
                             <Button 
                                ...
                                disabled={highestBidder?.team_id === myTeam?.team_id || timeLeft <= 0}
                             >
                                {highestBidder?.team_id === myTeam?.team_id ? "YOU ARE LEADING" : `CONFIRM BID ${formatCurrency(bidAmount)}`}
                                <Gavel size={28} weight="fill" className="ml-3 opacity-40" />
                             </Button>
                          </div>
                       </div>
                    </SectionCard>
         ...
         {/* BOTTOM SECTION: RESEARCH & DATA */}
         <div className="grid gap-6 lg:grid-cols-12 items-stretch mb-10">
             {/* Recently Sold */}
             <div className="lg:col-span-3">
                <SectionCard title="Recently Sold" padded={false} className="surface h-full">
                   <div className="divide-y divide-slate-100 max-h-[460px] overflow-y-auto no-scrollbar">
                      {recentlySold.length > 0 ? recentlySold.map((p, i) => (
                         <div key={i} className="p-4 flex items-center gap-3 hover:bg-slate-50 transition-colors">
                            <div className={cn("h-9 w-9 shrink-0 rounded-full flex items-center justify-center border border-slate-100 shadow-sm bg-slate-950 text-white text-[11px] font-bold overflow-hidden")}>
                               {p.team_logo ? (
                                  <img src={p.team_logo.startsWith("/") ? p.team_logo : `/uploads/${p.team_logo}`} alt="logo" className="h-full w-full object-cover" />
                               ) : (
                                  <span>{p.name?.substring(0, 2).toUpperCase()}</span>
                               )}
                            </div>
                            <div className="flex-1 min-w-0">
                               <p className="text-ui-xs font-bold text-slate-900 truncate capitalize leading-tight">{p.name}</p>
                               <p className="text-[10px] font-medium text-slate-500 truncate capitalize">{p.team_name}</p>
                            </div>
                            <div className="text-right">
                               <p className="text-[9px] font-bold text-slate-400 uppercase leading-none mb-1">Price</p>
                               <p className="text-ui-xs font-bold text-slate-900">{formatCurrency(p.amount)}</p>
                            </div>
                         </div>
                      )) : (
             ...
             <div className="lg:col-span-9">
                <SectionCard title="Franchise Squad Tracker" sub="Real-time roster distribution" className="surface h-full bg-white">
                   {teamStats.length > 0 ? (
                      <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
                         {teamStats.map((team, idx) => (
                            <div key={idx} className="bg-white rounded-md border border-slate-200 p-5 transition-all flex flex-col hover:shadow-md">
                               <div className="flex items-center justify-between mb-5 border-b border-slate-100 pb-3">
                                  <div className="flex items-center gap-2">
                                     <div className="h-7 w-7 shrink-0 rounded-full bg-slate-950 flex items-center justify-center text-[9px] font-bold text-white shadow-sm border border-slate-100 overflow-hidden">
                                        {team.logo_url ? (
                                           <img src={team.logo_url.startsWith("/") ? team.logo_url : `/uploads/${team.logo_url}`} alt="logo" className="h-full w-full object-cover" />
                                        ) : (
                                           team.team_name.substring(0, 2)
                                        )}
                                     </div>
                                     <h4 className="text-[11px] font-bold text-slate-900 capitalize">{team.team_name}</h4>
                                  </div>
                                ...
                                <div className="space-y-2.5 flex-1">
                                  ...
                                  {[
                                     { label: 'Batsman', int: team.batsman_os || 0, tot: team.batsman || 0 },
                                     { label: 'Keepers', int: team.keepers_os || 0, tot: team.keepers || 0 },
                                     { label: 'All Rounders', int: team.ar_os || 0, tot: team.ar || 0 },
                                     { label: 'Bowlers', int: team.bowlers_os || 0, tot: team.bowlers || 0 },
                                  ].map((role, ridx) => (
                                     <div key={ridx} className="grid grid-cols-12 text-[11px] font-semibold capitalize py-0.5">
                                        <div className="col-span-8 text-slate-600">{role.label}</div>
                                        <div className="col-span-2 text-center text-slate-900 font-bold">{role.int}</div>
                                        <div className="col-span-2 text-right text-slate-900 font-bold">{role.tot}</div>
                                     </div>
                                  ))}
                               </div>
                            </div>
