...
router.use((req, res, next) => {
  if (!['Admin', 'Super Admin'].includes(req.user.role)) return res.status(403).json({ error: 'Admin access required.' });
  next();
});
...
router.get('/teams', async (req, res) => {
  try {
    const [rows] = await db.query(`SELECT t.*, u.username, u.email FROM Teams t LEFT JOIN Users u ON t.user_id = u.user_id ORDER BY t.team_name`);
    res.json(rows);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
router.post('/teams', async (req, res) => {
  ...
  try {
    ...
    const [uRes] = await conn.query(`INSERT INTO Users (username, email, password_hash, role_id) VALUES (?, ?, ?, 3)`, [username.trim(), email.trim(), hash]);
    const budget = Number(total_budget) || 95000000;
    await conn.query(`INSERT INTO Teams (team_name, city, home_ground, total_budget, remaining_budget, owner_name, user_id) VALUES (?, ?, ?, ?, ?, ?, ?)`, [team_name, city, home_ground, budget, budget, owner_name, uRes.insertId]);
    await conn.commit();
...
router.put('/teams/:id', async (req, res) => {
  ...
  try {
    await conn.beginTransaction();
    await conn.query(`UPDATE Teams SET team_name=?, city=?, home_ground=?, total_budget=?, owner_name=? WHERE team_id=?`, [team_name, city, home_ground, total_budget, owner_name, req.params.id]);
    if (req.user.role === 'Super Admin' && (username || email || password)) {
      ...
      if (team?.user_id) {
        let uUpdates = []; let uValues = [];
        if (username) { uUpdates.push('username = ?'); uValues.push(username.trim()); }
        if (email)    { uUpdates.push('email = ?');    uValues.push(email.trim()); }
        if (password) { const hash = await bcrypt.hash(password, 12); uUpdates.push('password_hash = ?'); uValues.push(hash); }
        if (uUpdates.length > 0) { uValues.push(team.user_id); await conn.query(`UPDATE Users SET ${uUpdates.join(', ')} WHERE user_id = ?`, uValues); }
      }
    }
    await conn.commit();
...
router.delete('/teams/:id', async (req, res) => {
  ...
  try {
    ...
    if (team?.user_id) await conn.query('DELETE FROM Users WHERE user_id = ?', [team.user_id]);
    await conn.commit();
  ...
  try {
    const [rows] = await db.query(
      `SELECT p.*, ps.final_price, c.country_name, c.country_code, pc.category_name
       FROM Player_Sale ps JOIN Players p ON ps.player_id = p.player_id
       LEFT JOIN Countries c ON p.country_id = c.country_id
       LEFT JOIN Player_Category pc ON p.category_id = pc.category_id
       WHERE ps.team_id = ? ORDER BY p.name`,
      [req.params.teamId]
    );
    res.json(rows);
  } catch (err) { res.status(500).json({ error: err.message }); }
...
router.get('/players', async (req, res) => {
  try {
    const [rows] = await db.query(
      `SELECT p.*, pc.category_name, c.country_name, c.country_code, st.matches, st.runs_scored, st.wickets, st.avg_score, st.strike_rate
       FROM Players p
       LEFT JOIN Player_Category pc ON p.category_id = pc.category_id
       LEFT JOIN Countries c ON p.country_id = c.country_id
...
router.get('/players/available', async (req, res) => {
  ...
  try {
    const exclude = auction_id ? `AND p.player_id NOT IN (SELECT player_id FROM Auction_Pool WHERE auction_id = ${db.escape(auction_id)})` : '';
    const [rows] = await db.query(
      `SELECT p.player_id, p.name, p.role, p.base_price, pc.category_name
       FROM Players p LEFT JOIN Player_Category pc ON p.category_id = pc.category_id
       WHERE p.status = 'unsold' ${exclude} ORDER BY p.name`
    );
    res.json(rows);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
router.post('/players', upload.fields([{ name: 'image', maxCount: 1 }, { name: 'action_image', maxCount: 1 }, { name: 'video', maxCount: 1 }]), async (req, res) => {
  ...
  try {
    ...
    const [pRes] = await conn.query(
      `INSERT INTO Players (name, age, role, base_price, country_id, category_id, batting_style, bowling_style, image_url, action_image_url, video_url, added_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    ...
    await conn.query(`INSERT INTO Player_Stats (player_id, matches, runs_scored, wickets, avg_score, strike_rate) VALUES (?, ?, ?, ?, ?, ?)`, [pRes.insertId, matches || 0, runs_scored || 0, wickets || 0, avg_score || 0, strike_rate || 0]);
    await conn.commit();
  ...
  try {
    ...
    await conn.query(
      `UPDATE Players SET name=?, age=?, role=?, base_price=?, country_id=?, category_id=?, batting_style=?, bowling_style=?
       ${imageUrl ? ', image_url=?' : ''} ${actionImageUrl ? ', action_image_url=?' : ''} ${videoUrl ? ', video_url=?' : ''} WHERE player_id=?`,
    ...
    await conn.query(
      `INSERT INTO Player_Stats (player_id, matches, runs_scored, wickets, avg_score, strike_rate) VALUES (?, ?, ?, ?, ?, ?)
       ON DUPLICATE KEY UPDATE matches=VALUES(matches), runs_scored=VALUES(runs_scored), wickets=VALUES(wickets), avg_score=VALUES(avg_score), strike_rate=VALUES(strike_rate)`,
...
router.post('/players/:id/direct-assign', async (req, res) => {
  ...
  if (!team_id || !price || !auction_id || !season) return res.status(400).json({ error: 'Team, price, auction, and season are required.' });
  try {
    await db.query('CALL Sell_Player(?, ?, ?, ?, ?)', [req.params.id, team_id, auction_id, price, season]);
...
router.get('/bids/:playerId', async (req, res) => {
  ...
  try {
    const [rows] = await db.query(
      `SELECT b.*, t.team_name FROM Bids b JOIN Teams t ON b.team_id = t.team_id
       WHERE b.player_id = ? ${auction_id ? 'AND b.auction_id = ?' : ''} ORDER BY b.bid_amount DESC LIMIT 30`,
  ...
  try {
    const [rows] = await db.query(
      `SELECT ap.*, p.name, p.role, p.base_price, p.image_url, pc.category_name, c.country_name, c.country_code
       ...
       LEFT JOIN Player_Category pc ON p.category_id = pc.category_id
       LEFT JOIN Countries c ON p.country_id = c.country_id
       WHERE p.status != 'sold' ORDER BY ap.auction_id DESC, ap.lot_number ASC`
    );
    res.json(rows);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
router.post('/next-player', async (req, res) => {
  ...
  try {
    ...
    if (status === 'upcoming') await conn.query('UPDATE Auction SET status = ? WHERE auction_id = ?', ['active', auction_id]);
    ...
    const [next] = await conn.query(`SELECT pool_id, player_id FROM Auction_Pool WHERE auction_id = ? AND status = 'waiting' ORDER BY lot_number ASC LIMIT 1`, [auction_id]);
    if (next.length === 0) { await conn.commit(); return res.json({ message: 'No more players in pool.', done: true }); }
    ...
    if (io) {
      const [details] = await db.query(
        `SELECT p.*, pc.category_name, c.country_name, c.country_code FROM Players p
         LEFT JOIN Player_Category pc ON p.category_id = pc.category_id
         LEFT JOIN Countries c ON p.country_id = c.country_id WHERE p.player_id = ?`,
        [player_id]
      );
      const playerDetails = details[0];
      auctionTimers[auction_id] = { timeLeft: 60, isActive: false, currentPlayer: playerDetails, highestBid: Number(playerDetails.base_price), highestBidder: null };
...
router.post('/sell-player', async (req, res) => {
  ...
  try {
    ...
    await conn.commit();
    const io = req.app.get('io');
    if (io) io.to(`auction_${auction.auction_id}`).emit('player_sold', { player_id: active.player_id, team_id: active.highest_bidder_id, amount: active.current_bid });
...
router.post('/reset-auction', async (req, res) => {
  ...
  try {
    ...
    const io = req.app.get('io');
    if (io) io.emit('auction_sync', { timeLeft: 0, isActive: false, currentPlayer: null, highestBid: 0, highestBidder: null });
    await conn.commit();
