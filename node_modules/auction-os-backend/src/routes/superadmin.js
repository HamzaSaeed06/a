...
router.get('/overview-stats', async (req, res) => {
  try {
    ...
    res.json({ total_users, active_users, total_teams, total_players, total_countries, total_categories, total_auctions, total_seasons: total_auctions, live_auctions });
...
router.put('/users/:id', async (req, res) => {
  ...
  try {
    let updates = [];
    let values = [];
    if (typeof is_active !== 'undefined') { updates.push('is_active = ?'); values.push(is_active ? 1 : 0); }
    if (role_name) {
      const [[role]] = await db.query('SELECT role_id FROM Roles WHERE role_name = ?', [role_name]);
      if (!role) return res.status(400).json({ error: 'Invalid role.' });
      updates.push('role_id = ?'); values.push(role.role_id);
    }
    if (password) { const hash = await bcrypt.hash(password, 12); updates.push('password_hash = ?'); values.push(hash); }
    if (username) { updates.push('username = ?'); values.push(username.trim()); }
    if (email)    { updates.push('email = ?');    values.push(email.trim()); }
    if (updates.length > 0) { values.push(req.params.id); await db.query(`UPDATE Users SET ${updates.join(', ')} WHERE user_id = ?`, values); }
...
router.delete('/users/:id', async (req, res) => {
  try {
    const [[user]] = await db.query('SELECT r.role_name FROM Users u JOIN Roles r ON u.role_id = r.role_id WHERE u.user_id = ?', [req.params.id]);
...
router.post('/categories', async (req, res) => {
  ...
  try {
    ...
    await db.query(`INSERT INTO Player_Category (category_name, min_price, max_price) VALUES (?, ?, ?)`, [category_name, finalMin, max_price || 999999999]);
...
router.put('/categories/:id', async (req, res) => {
  ...
  try {
    const finalMin = base_price || min_price || 0;
    await db.query(`UPDATE Player_Category SET category_name = ?, min_price = ?, max_price = ? WHERE category_id = ?`, [category_name, finalMin, max_price || 999999999, req.params.id]);
...
router.get('/countries', async (req, res) => {
  try {
    const [rows] = await db.query(
      `SELECT c.*, c.country_code, c.region AS dial_code,
...
router.get('/auctions', async (req, res) => {
  try {
    const [rows] = await db.query(
      `SELECT a.*, COALESCE(a.location, a.venue, '') AS location,
...
router.post('/auctions', async (req, res) => {
  ...
  try {
    ...
    await db.query(
      `INSERT INTO Auction (auction_name, season, auction_date, venue, location, total_budget_per_team, status, description) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [auction_name || `Auction ${yr}`, yr, dt, location || null, location || null, total_budget_per_team || null, status || 'upcoming', description || null]
    );
...
router.put('/auctions/:id', async (req, res) => {
  ...
  try {
    ...
    const dt = auction_date || `${yr}-01-01`;
    await db.query(
      `UPDATE Auction SET auction_name=?, season=?, auction_date=?, venue=?, location=?, total_budget_per_team=?, status=?, description=? WHERE auction_id=?`,
      [auction_name || `Auction ${yr}`, yr, dt, location || null, location || null, total_budget_per_team || null, status || 'upcoming', description || null, req.params.id]
    );
